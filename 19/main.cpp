...
LL calc(VECI cost, VECI rob, VECI res, LL time)
{
    ...
    if(res[0] >= oreCost                             && orR <= clR*2) { score = std::max(score, calc(cost, {orR+1, clR, obR, geR}, {ore - oreCost, clay, obs, geo}, time+1)); }
    if(res[0] >= clayCost                            && clR <= orR * 2 && clR <= obR*2) { score = std::max(score, calc(cost, {orR, clR+1, obR, geR}, {ore - clayCost, clay, obs, geo}, time+1)); }
    if(res[0] >= obsOreCost && res[1] >= obsClayCost && obR <= geR * 2) { score = std::max(score, calc(cost, {orR, clR, obR+1, geR}, {ore - obsOreCost, clay - obsClayCost, obs, geo}, time+1)); }
