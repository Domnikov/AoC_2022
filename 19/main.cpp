...
{
    P_LINE;
    if(time >= 24){if(res[3]){P(res[3]);} return res[3];}
    LL oreCost     = cost[0];
    LL clayCost    = cost[1];
    ...
    if(res[0] >= clayCost                            && clR <= (orR+1) * 3 && clR <= (obR+1)*3) { score = std::max(score, calc(cost, {orR, clR+1, obR, geR}, {ore - clayCost, clay, obs, geo}, time+1)); }
